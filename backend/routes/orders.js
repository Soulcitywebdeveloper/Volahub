...
// Email is optional — if not configured, orders still work
...
const sendMail = async (fn, ...args) => {
  if (!emailService || !emailService[fn]) return;
  try { await emailService[fn](...args); }
  catch (e) { console.error(`Email ${fn} failed:`, e.message); }
};

const router = express.Router();

// ⚠️  IMPORTANT: Specific named routes MUST come before /:id
// Otherwise Express matches 'my-orders' and 'track' as IDs

// ── PUBLIC: Track order by order number + email ──────────────────────────────
router.post('/track', async (req, res) => {
  try {
    ...
    if (order.user.email.toLowerCase() !== email.toLowerCase()) {
      return res.status(403).json({ success: false, message: 'Email address does not match this order.' });
    }
...
router.post('/', protect, async (req, res) => {
  try {
    ...
    for (const item of items) {
      ...
      if (product.stock.quantity < item.quantity) return res.status(400).json({ success: false, message: `Insufficient stock for ${product.name}.` });
    ...
    for (const item of orderItems) {
      await Product.findByIdAndUpdate(item.product, { $inc: { 'stock.quantity': -item.quantity, salesCount: item.quantity } });
    }
    ...
    sendMail('sendOrderConfirmation', order, req.user);
    sendMail('sendAdminNewOrderAlert', order, req.user);
...
router.patch('/:id/status', protect, adminOnly, async (req, res) => {
  try {
    ...
    sendMail('sendOrderStatusUpdate', order, order.user, status, note);
